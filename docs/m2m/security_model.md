...
- Foundation 8 adds a local offline replay/idempotency registry (`coin/m2m_replay_registry.py`) that stores only hashed exchange identifiers and message IDs. It MUST NOT be treated as an L28 ledger, consensus, wallet, or settlement authority, and MUST NOT be assumed encrypted or tamper-proof. See [replay_registry_v0.1.md](replay_registry_v0.1.md).
- Foundation 9 optionally integrates Foundation 8 into the Foundation 7 CLI via explicit registry flags, emitting a deterministic admission report after verify-before-registry ordering. Without registry flags, Foundation 7 behavior is unchanged. Admission success does not imply settlement finality, service completion, or spending authority. See [admission_cli_v0.1.md](admission_cli_v0.1.md).
- Foundation 10 adds a strictly read-only replay-registry auditor (`coin/m2m_registry_audit.py`, `coin/m2m_registry_audit_cli.py`) that inspects existing registries without creating, modifying, repairing, or admitting state. Audit success is local integrity evidence only and does not prove settlement acceptance or network validity. See [registry_audit_v0.1.md](registry_audit_v0.1.md).
- Foundation 12 adds offline replay-registry backup and verified recovery (`coin/m2m_registry_backup.py`, `coin/m2m_registry_backup_cli.py`). Backups are not encrypted, not authenticated, and remain locally correlatable hash-only SQLite artifacts. Restore creates a new destination only; it does not activate, replace, or delete operational registries. See [registry_backup_recovery_v0.1.md](registry_backup_recovery_v0.1.md).
