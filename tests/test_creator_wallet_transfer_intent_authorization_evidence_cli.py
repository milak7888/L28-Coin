...
from coin import creator_wallet_transfer_intent_authorization_evidence_cli as cli
from coin.creator_wallet_transfer_intent_authorization_evidence import (
    MAX_EVIDENCE_BYTES,
...
def _result(*, ok: bool = True, code: str = "ok"):
    return CreatorWalletTransferIntentAuthorizationEvidenceResult(
        ...
        evidence_sha256=HEX_A if ok else "",
        authorization_sha256=HEX_B if ok else "",
        authorization_id=HEX_A if ok else "",
    )
...
class CreatorWalletTransferIntentAuthorizationEvidenceCliTests(unittest.TestCase):
    def test_valid_result_is_reported(self) -> None:
        with mock.patch.object(
            cli,
            "verify_creator_wallet_transfer_intent_authorization_evidence_file",
            return_value=_result(),
        ):
            code, output, _ = _invoke(["evidence.json"])

        self.assertEqual(code, cli.EXIT_OK)
        report = json.loads(output)
        self.assertTrue(report["ok"])
        self.assertEqual(report["evidence_sha256"], HEX_A)
        self.assertEqual(report["authorization_id"], HEX_A)
    ...
    def test_pretty_report_is_logically_equal(self) -> None:
        with mock.patch.object(
            ...
            compact = _invoke(["evidence.json"])
            pretty = _invoke(["evidence.json", "--pretty"])
    ...
    def test_verification_failure_uses_verification_exit(self) -> None:
        with mock.patch.object(
            cli,
            "verify_creator_wallet_transfer_intent_authorization_evidence_file",
            return_value=_result(ok=False, code="authorization_invalid"),
        ):
            code, output, _ = _invoke(["evidence.json"])
    ...
    def test_regular_file_calls_core_verifier(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.object(
                ...
                actual = cli.verify_creator_wallet_transfer_intent_authorization_evidence_file(
                    str(path)
                )
            ...
            for path in (root / "missing.json", root, symlink):
                with self.subTest(path=path):
                    with self.assertRaises(cli._CliError) as raised:
                        cli.verify_creator_wallet_transfer_intent_authorization_evidence_file(
                            str(path)
                        )
    ...
    def test_report_id_is_deterministic_and_body_bound(self) -> None:
        ...
        changed = cli._build_report(_result(ok=False, code="report_invalid"))
    ...
    def test_unexpected_exception_is_sanitized(self) -> None:
        with mock.patch.object(
            ...
            code, output, error = _invoke(["evidence.json"])
