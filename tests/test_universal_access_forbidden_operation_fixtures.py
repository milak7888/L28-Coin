...
"""Foundation 89 — isolated forbidden-operation fixture validation.
...
broadcast, autonomous spend, or a production Universal Access Interface
runtime.
"""
...
FOR_DIR = FIXTURE_ROOT / "forbidden_operations"
...
OVERRIDE_PARAM_KEYS = frozenset(
    {
        "adapter_override_canonical_height",
        "adapter_override_historically_mined_l28",
        "adapter_override_hard_cap_l28",
...
CASE_ID_RE = re.compile(r"^UAI-CONF-v0\.1-FOR-(POS|NEG|BND|FCL)-\d{3}$")

PLANNED_IDS = (
    "UAI-CONF-v0.1-FOR-NEG-001",
    "UAI-CONF-v0.1-FOR-NEG-002",
    "UAI-CONF-v0.1-FOR-FCL-001",
)
...
def discover_fixture_paths() -> list[Path]:
    by_id = {p.stem: p for p in FOR_DIR.glob("*.json")}
...
def evaluate_forbidden_operation(fx: dict[str, Any]) -> dict[str, Any]:
    req = fx["request"]
    ...
    if req["operation"] in FORBIDDEN_OPERATIONS:
        return {
            "outcome": "reject",
            "ok": False,
...
def evaluate_fixture(fx: dict[str, Any]) -> dict[str, Any]:
    return evaluate_forbidden_operation(fx)


class TestUniversalAccessForbiddenOperationFixtures(unittest.TestCase):
    @classmethod
    ...
    def test_discovers_only_forbidden_operations_dir_and_exactly_3(self) -> None:
        self.assertTrue(FOR_DIR.is_dir())
        self.assertEqual(len(self.paths), 3)
        self.assertEqual({p.parent for p in self.paths}, {FOR_DIR})
    ...
    def test_structure_and_mapping(self) -> None:
        for fx in self.fixtures:
            ...
            self.assertEqual(fx["area"], "forbidden_operations")
    ...
    def test_neg_sign_and_broadcast_unsupported(self) -> None:
        fx = self.by_id["UAI-CONF-v0.1-FOR-NEG-001"]
        self.assertEqual(fx["request"]["operation"], "sign_and_broadcast")
    ...
    def test_neg_autonomous_spend_unsupported(self) -> None:
        fx = self.by_id["UAI-CONF-v0.1-FOR-NEG-002"]
        self.assertEqual(fx["request"]["operation"], "autonomous_spend")
        self.assertIs(fx["request"]["params"]["spend_authorized"], True)
        self.assertEqual(fx["expected"]["code"], "operation_unsupported")
        self.assertIs(fx["expected"]["result"]["spend_authorized"], False)

    def test_fcl_adapter_supply_height_override_forbidden(self) -> None:
        fx = self.by_id["UAI-CONF-v0.1-FOR-FCL-001"]
        params = fx["request"]["params"]
        self.assertIn("adapter_override_canonical_height", params)
        self.assertIn("adapter_override_historically_mined_l28", params)
        self.assertEqual(fx["expected"]["code"], "adapter_override_forbidden")
        result = fx["expected"]["result"]
        self.assertIs(result["consensus_overridden"], False)
        self.assertIs(result["protocol_mutated"], False)
