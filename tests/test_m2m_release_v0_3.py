...
"""
Offline tests for Foundation 15 L28 M2M v0.3.0 release candidate manifest.

Independently verifies dual historical anchoring, artifact inventory, contracts,
reference-workflow freeze, and manifest_id determinism.
"""
...
from coin.m2m_reference_workflow import (
    ...
    CLI_VERSION as WORKFLOW_CLI_VERSION,
    DOMAIN_REPORT as WORKFLOW_DOMAIN_REPORT,
    EXIT_FAIL as WORKFLOW_EXIT_FAIL,
    EXIT_INTERNAL as WORKFLOW_EXIT_INTERNAL,
    EXIT_PASS as WORKFLOW_EXIT_PASS,
    EXIT_USAGE as WORKFLOW_EXIT_USAGE,
    PROFILE as WORKFLOW_PROFILE,
    REPORT_VERSION as WORKFLOW_REPORT_VERSION,
    STAGE_ORDER as WORKFLOW_STAGE_ORDER,
    STABLE_CODES as WORKFLOW_CODES,
)
...
MANIFEST_PATH = ROOT / "docs" / "m2m" / "release_manifest_v0.3.json"
DOMAIN_MANIFEST_V01 = b"L28-M2M-V0.1-RELEASE-MANIFEST\x00"
DOMAIN_MANIFEST_V02 = b"L28-M2M-V0.2-RELEASE-MANIFEST\x00"
DOMAIN_MANIFEST = b"L28-M2M-V0.3-RELEASE-MANIFEST\x00"
...
HISTORICAL_V02_TAG = "l28-m2m-v0.2.0"
HISTORICAL_V02_COMMIT = "4ea3c1b38a77d06f907baefb6783231f9bc84ce9"
HISTORICAL_V02_MANIFEST_PATH = "docs/m2m/release_manifest_v0.2.json"
...
MANIFEST_EXCLUDE_ONLY = "docs/m2m/release_manifest_v0.3.json"
...
RELEASE_NOTES_V03 = "docs/m2m/release_notes_v0.3.md"
COMPATIBILITY_V03 = "docs/m2m/release_compatibility_v0.3.md"
RELEASE_TEST_V01 = "tests/test_m2m_release_candidate.py"
RELEASE_TEST_V02 = "tests/test_m2m_release_v0_2.py"
RELEASE_TEST_V03 = "tests/test_m2m_release_v0_3.py"
...
TAG_EXISTS_CLAIMS = (
    "tag `l28-m2m-v0.3.0` has been",
    "github release for v0.3.0",
    "published at tag l28-m2m-v0.3.0",
)
...
def _collect_expected_artifact_paths() -> List[str]:
    ...
    paths.append(ROOT / HISTORICAL_V02_MANIFEST_PATH)
...
def _expected_role(path: str) -> str:
    if path in {HISTORICAL_V01_MANIFEST_PATH, HISTORICAL_V02_MANIFEST_PATH}:
...
class ReleaseV03ManifestStructureTests(unittest.TestCase):
    ...
    def test_release_identity(self) -> None:
        manifest = _load_manifest()
        self.assertEqual(manifest["manifest_version"], "l28-m2m-release-manifest/v0.3")
        self.assertEqual(manifest["release_name"], "L28 M2M v0.3.0")
        self.assertEqual(manifest["release_version"], "0.3.0")
        self.assertEqual(manifest["intended_tag"], "l28-m2m-v0.3.0")
        self.assertEqual(manifest["status"], "frozen")
        self.assertEqual(
            manifest["release_compatibility_profile"],
            "l28-m2m-release-compatibility/v0.3",
        )
    ...
    def test_prior_release_anchor(self) -> None:
        ...
        self.assertEqual(prior["tag"], HISTORICAL_V02_TAG)
        self.assertEqual(prior["commit"], HISTORICAL_V02_COMMIT)
        self.assertEqual(prior["manifest_id"], HISTORICAL_V02_MANIFEST_ID)
        self.assertEqual(prior["manifest_sha256"], HISTORICAL_V02_MANIFEST_SHA256)
        self.assertEqual(prior["manifest_path"], HISTORICAL_V02_MANIFEST_PATH)
...
class ReleaseV03HistoricalManifestTests(unittest.TestCase):
    ...
    def test_v02_manifest_byte_identity(self) -> None:
    ...
    def test_v02_manifest_id_unchanged(self) -> None:
        ...
        without_id = {k: v for k, v in manifest.items() if k != "manifest_id"}
        expected = hashlib.sha256(
            DOMAIN_MANIFEST_V02 + canonical_bytes(without_id)
        ).hexdigest()
        self.assertEqual(manifest["manifest_id"], HISTORICAL_V02_MANIFEST_ID)
        self.assertEqual(expected, HISTORICAL_V02_MANIFEST_ID)

    def test_historical_manifests_included(self) -> None:
        ...
        self.assertEqual(index[HISTORICAL_V02_MANIFEST_PATH]["role"], "historical_release_manifest")
        self.assertEqual(index[HISTORICAL_V02_MANIFEST_PATH]["sha256"], HISTORICAL_V02_MANIFEST_SHA256)


class ReleaseV03ManifestIdTests(unittest.TestCase):
    ...
    def test_manifest_id_changes_when_body_tampered(self) -> None:
        ...
        tampered["release_version"] = "0.3.1"
    ...
    def test_manifest_excludes_only_itself(self) -> None:
        ...
        self.assertIn(HISTORICAL_V02_MANIFEST_PATH, paths)


class ReleaseV03ArtifactInventoryTests(unittest.TestCase):
    ...
    def test_release_documents_present(self) -> None:
        ...
        for rel, role in (
            ...
            (RELEASE_NOTES_V03, "release_document"),
            (COMPATIBILITY_V03, "release_document"),
            (RELEASE_TEST_V01, "conformance_test"),
            (RELEASE_TEST_V02, "conformance_test"),
            (RELEASE_TEST_V03, "conformance_test"),
        ):
            self.assertIn(rel, index)
            self.assertEqual(index[rel]["role"], role)


class ReleaseV03ContractTests(unittest.TestCase):
    ...
    def test_domain_separators(self) -> None:
        ...
        self.assertEqual(
            ...
            _manifest_domain_literal(DOMAIN_MANIFEST_V02),
        )
        self.assertEqual(
            separators["release_manifest_v0_3"],
            _manifest_domain_literal(DOMAIN_MANIFEST),
        )
        self.assertEqual(
            separators["reference_workflow_report"],
            _manifest_domain_literal(WORKFLOW_DOMAIN_REPORT),
        )
        self.assertEqual(
    ...
    def test_reference_workflow_contract(self) -> None:
        workflow = _load_manifest()["public_contracts"]["reference_workflow"]
        self.assertEqual(workflow["profile"], WORKFLOW_PROFILE)
        self.assertEqual(workflow["report_version"], WORKFLOW_REPORT_VERSION)
        self.assertEqual(
            workflow["report_id_domain"],
            _manifest_domain_literal(WORKFLOW_DOMAIN_REPORT),
        )
        self.assertEqual(workflow["cli_version"], WORKFLOW_CLI_VERSION)
        self.assertEqual(set(workflow["approved_input_modes"]), set(APPROVED_INPUT_MODES))
        self.assertTrue(workflow["require_terminal_exact_bool"])
        self.assertEqual(tuple(workflow["stage_order"]), WORKFLOW_STAGE_ORDER)
        self.assertEqual(workflow["initial_admission_code"], "recorded_new")
        self.assertEqual(workflow["source_idempotency_code"], "already_recorded")
        self.assertEqual(workflow["restored_idempotency_code"], "already_recorded")
        self.assertEqual(workflow["source_audit_code"], "registry_healthy")
        self.assertEqual(workflow["backup_code"], "backup_created")
        self.assertEqual(workflow["restore_code"], "restore_created")
        self.assertEqual(workflow["restored_audit_code"], "registry_healthy")
        self.assertTrue(workflow["temporary_state_outside_repository"])
        self.assertTrue(workflow["temporary_state_removed"])
        exits = workflow["exit_codes"]
        self.assertEqual(exits["pass"], WORKFLOW_EXIT_PASS)
        self.assertEqual(exits["fail"], WORKFLOW_EXIT_FAIL)
        self.assertEqual(exits["usage"], WORKFLOW_EXIT_USAGE)
        self.assertEqual(exits["internal"], WORKFLOW_EXIT_INTERNAL)
        self.assertEqual(set(workflow["stable_result_codes"]), set(WORKFLOW_CODES))
...
class ReleaseV03ImplementationContractTests(unittest.TestCase):
...
class ReleaseV03VectorTests(unittest.TestCase):
...
class ReleaseV03DocumentTests(unittest.TestCase):
    def test_release_notes_required_content(self) -> None:
        text = (ROOT / RELEASE_NOTES_V03).read_text(encoding="utf-8")
        self.assertIn("v0.3.0", text)
        self.assertIn("release candidate", text.lower())
        self.assertIn("l28-m2m/v0.1", text)
        self.assertIn("l28-m2m-reference-workflow/v0.1", text)
        self.assertIn(HISTORICAL_V01_TAG, text)
        self.assertIn(HISTORICAL_V02_TAG, text)
        self.assertIn("l28-m2m-v0.3.0", text)
    ...
    def test_compatibility_document_distinctions(self) -> None:
        text = (ROOT / COMPATIBILITY_V03).read_text(encoding="utf-8")
        self.assertIn("l28-m2m-release-compatibility/v0.3", text)
        self.assertIn("l28-m2m/v0.1", text)
        self.assertIn("l28-m2m-reference-workflow/v0.1", text)
        self.assertIn(HISTORICAL_V01_TAG, text)
        self.assertIn(HISTORICAL_V02_TAG, text)

    def test_readme_links_v03_honestly(self) -> None:
        text = (ROOT / README_PATH).read_text(encoding="utf-8")
        self.assertIn("release_manifest_v0.3.json", text)
        self.assertIn("release_notes_v0.3.md", text)
        self.assertIn("release_compatibility_v0.3.md", text)
        self.assertIn("release candidate", text.lower())
        self.assertIn("l28-m2m/v0.1", text)
        self.assertIn("l28-m2m-reference-workflow/v0.1", text)
    ...
    def test_docs_do_not_claim_v03_tag_or_release_exists(self) -> None:
        for rel in (RELEASE_NOTES_V03, COMPATIBILITY_V03, README_PATH):
            text = (ROOT / rel).read_text(encoding="utf-8").lower()
            self.assertNotIn("github release for l28 m2m v0.3.0", text)
            self.assertNotIn("tag l28-m2m-v0.3.0 is published", text)

    def test_no_private_material_in_release_surface(self) -> None:
        paths = [
            MANIFEST_PATH,
            ROOT / RELEASE_NOTES_V03,
            ROOT / COMPATIBILITY_V03,
            ROOT / README_PATH,
        ]
        for path in paths:
...
class ReleaseV03DeterminismTests(unittest.TestCase):
    ...
    def test_release_docs_do_not_embed_v03_manifest_id(self) -> None:
        manifest_id = _load_manifest()["manifest_id"]
        for rel in (RELEASE_NOTES_V03, COMPATIBILITY_V03):
