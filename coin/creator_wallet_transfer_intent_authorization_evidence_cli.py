"""Explicit-path CLI for offline transfer-intent authorization evidence."""
...
REPORT_DOMAIN = (EVIDENCE_REPORT_VERSION + "\x00").encode("utf-8")
...
def _build_report(
    result: CreatorWalletTransferIntentAuthorizationEvidenceResult,
) -> dict[str, Any]:
    body: dict[str, Any] = {
...
def _failure(code: str) -> CreatorWalletTransferIntentAuthorizationEvidenceResult:
    return CreatorWalletTransferIntentAuthorizationEvidenceResult(False, code)
...
def verify_creator_wallet_transfer_intent_authorization_evidence_file(
    path_text: str,
) -> CreatorWalletTransferIntentAuthorizationEvidenceResult:
    raw = _read_explicit_file(path_text)
    return verify_creator_wallet_transfer_intent_authorization_evidence_json(raw)


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="creator-wallet-transfer-intent-authorization-evidence",
        description="Verify one explicit offline authorization-evidence JSON file.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        result = verify_creator_wallet_transfer_intent_authorization_evidence_file(
            args.evidence_file
        )
